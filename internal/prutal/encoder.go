...
func (e *Encoder) AppendStruct(b []byte, base unsafe.Pointer, s *desc.StructDesc, encodeLen bool, maxdepth int) (_ []byte, err error) {
	...
	for _, f := range s.Fields {
		...
		// Skip zero values, which also covers nil pointers and maps. The kind
		// was resolved when the field was finalized. This is an if chain on
		// purpose: a switch over the kinds would be compiled into a binary
		// search over the case values, while here the common kinds come
		// first and pay for a single compare.
