...
func AppendMap_VarintU32_VarintU32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_VarintU32_VarintU64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_VarintU32_ZigZag32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_VarintU32_ZigZag64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_VarintU32_Fixed32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint32]uint32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_VarintU32_Fixed64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint32]uint64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_VarintU32_Bool(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint32]bool)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_VarintU32_Bytes(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint32][]byte)(p)
	if len(m) == 0 {
		return b
...
func AppendMap_VarintU32_String(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_VarintU64_VarintU32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_VarintU64_VarintU64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_VarintU64_ZigZag32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_VarintU64_ZigZag64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_VarintU64_Fixed32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint64]uint32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_VarintU64_Fixed64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint64]uint64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_VarintU64_Bool(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint64]bool)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_VarintU64_Bytes(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint64][]byte)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_VarintU64_String(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_ZigZag32_VarintU32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_ZigZag32_VarintU64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_ZigZag32_ZigZag32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[int32]int32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_ZigZag32_ZigZag64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[int32]int64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_ZigZag32_Fixed32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[int32]uint32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_ZigZag32_Fixed64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[int32]uint64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_ZigZag32_Bool(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[int32]bool)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_ZigZag32_Bytes(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[int32][]byte)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_ZigZag32_String(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_ZigZag64_VarintU32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_ZigZag64_VarintU64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_ZigZag64_ZigZag32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[int64]int32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_ZigZag64_ZigZag64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[int64]int64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_ZigZag64_Fixed32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[int64]uint32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_ZigZag64_Fixed64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[int64]uint64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_ZigZag64_Bool(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[int64]bool)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_ZigZag64_Bytes(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[int64][]byte)(p)
	if len(m) == 0 {
		return b
...
func AppendMap_ZigZag64_String(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_Fixed32_VarintU32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint32]uint32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed32_VarintU64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint32]uint64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed32_ZigZag32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint32]int32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed32_ZigZag64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint32]int64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed32_Fixed32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint32]uint32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed32_Fixed64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint32]uint64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed32_Bool(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint32]bool)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed32_Bytes(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint32][]byte)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed32_String(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint32]string)(p)
	if len(m) == 0 {
		return b
...
func AppendMap_Fixed64_VarintU32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint64]uint32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed64_VarintU64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint64]uint64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed64_ZigZag32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint64]int32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed64_ZigZag64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint64]int64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed64_Fixed32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint64]uint32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed64_Fixed64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint64]uint64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed64_Bool(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint64]bool)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed64_Bytes(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint64][]byte)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Fixed64_String(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[uint64]string)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
// Bool key encoders
func AppendMap_Bool_VarintU32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[bool]uint32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Bool_VarintU64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[bool]uint64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Bool_ZigZag32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[bool]int32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Bool_ZigZag64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[bool]int64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Bool_Fixed32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[bool]uint32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Bool_Fixed64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[bool]uint64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Bool_Bool(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[bool]bool)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Bool_Bytes(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[bool][]byte)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_Bool_String(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[bool]string)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
// String key encoders
func AppendMap_String_VarintU32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_String_VarintU64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_String_ZigZag32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_String_ZigZag64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
...
func AppendMap_String_Fixed32(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[string]uint32)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_String_Fixed64(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[string]uint64)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_String_Bool(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[string]bool)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_String_Bytes(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
	m := *(*map[string][]byte)(p)
	for k, v := range m {
		b = AppendVarint(b, tag)
...
func AppendMap_String_String(b []byte, id int32, p unsafe.Pointer) []byte {
	tag := uint64(id)<<3 | uint64(TypeBytes)
