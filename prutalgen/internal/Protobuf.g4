// This file originates from github.com/antlr/grammars-v4 Protobuf3.g4
...
proto
    : edition? (importStatement | packageStatement | optionStatement | topLevelDef | emptyStatement)* EOF
    ;

// Edition

edition
    : (SYNTAX | EDITION) EQ strLit SEMI
    ;
...
field
    : fieldLabel? fieldType fieldName EQ fieldNumber (LB fieldOptions RB)? SEMI
    ;
...
mapField
    : MAP LT keyType COMMA fieldType GT fieldName EQ fieldNumber (LB fieldOptions RB)? SEMI
    ;
...
blockLit
    : LC (ident COLON constant (SEMI | COMMA)? )* RC
    ;
...
// comments
// keep it in channel 2, may use it later to check empty lines
WS
    : [ \t\r\n\u000C]+ -> channel(2)
    ;
