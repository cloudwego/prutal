...
type GoCodeGen struct {
	Format bool
	// Module is the Go module prefix removed from import-based output paths.
	Module string
...
// SourcePath returns the output path for a generated file.
func (g *GoCodeGen) SourcePath(p *Proto, gt GenPathType, out string, suffix string) string {
	fn, _ := g.sourcePath(p, gt, out, suffix)
	return fn
}

func (g *GoCodeGen) sourcePath(p *Proto, gt GenPathType, out string, suffix string) (string, error) {
	if g.Module != "" && gt == GenBySourceRelative {
		return "", fmt.Errorf("cannot use module= with paths=source_relative")
	}
	generatedName := ""
	switch gt {
	case GenByImport:
		// same name as proto file
		protoName := p.protoName
		if protoName == "" {
			protoName = p.ProtoFile
		}
		fn := trimProtoExtension(filepath.Base(protoName)) + suffix
		// file path uses import path
		generatedName = path.Join(p.GoImport, fn)
		if g.Module != "" {
			prefix := g.Module + "/"
			if !strings.HasPrefix(generatedName, prefix) {
				return "", fmt.Errorf(
					"%s: generated file does not match prefix %q",
					generatedName, g.Module)
			}
	...
	case GenBySourceRelative:
		...
		if p.protoName == "" {
			return trimProtoExtension(p.ProtoFile) + suffix, nil
		} else {
			generatedName = trimProtoExtension(p.protoName) + suffix
		}

	default:
		panic("unknown GenPathType: " + gt)
	}
	return joinOutputPath(out, generatedName)
}

func trimProtoExtension(name string) string {
	for _, ext := range []string{".proto", ".protodevel"} {
...
func joinOutputPath(out, generatedName string) (string, error) {
	nativeName := filepath.FromSlash(generatedName)
	if generatedName == "" || path.IsAbs(generatedName) || path.Clean(generatedName) != generatedName ||
		generatedName == ".." || strings.HasPrefix(generatedName, "../") ||
		strings.ContainsRune(generatedName, '\\') || filepath.IsAbs(nativeName) ||
		filepath.VolumeName(nativeName) != "" {
		return "", fmt.Errorf("invalid generated file path %q", generatedName)
	}
	return filepath.Join(out, nativeName), nil
}

// ValidateOutputPaths validates every output path before generator emission.
func (g *GoCodeGen) ValidateOutputPaths(protos []*Proto, gt GenPathType, out string) error {
	seen := make(map[string]*Proto, len(protos))
	for _, p := range protos {
		fn, err := g.sourcePath(p, gt, out, ".pb.go")
		if err != nil {
			return err
		}
		key, err := filepath.Abs(fn)
		if err != nil {
			return err
		}
		if previous := seen[key]; previous != nil {
			...
			return fmt.Errorf("%s: tried to write the same file twice for %q and %q",
				filepath.ToSlash(fn), previous.protoName, p.protoName)
		}
		seen[key] = p
	}
	return nil
